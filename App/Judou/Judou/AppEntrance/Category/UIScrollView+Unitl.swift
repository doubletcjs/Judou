...
extension UIScrollView {
    ...
    func setupRefresh(_ target: AnyObject, _ headerAction: Selector?, _ footerAction: Selector?) -> Void {
        ...
        DispatchQueue.main.async(execute: {
            self.mj_header.beginRefreshingCompletionBlock = {
        ...
        if footerAction != nil {
            footer.setRefreshingTarget(target, refreshingAction: footerAction)
        }
        ...
        DispatchQueue.main.async(execute: {
            self.mj_footer.beginRefreshingCompletionBlock = {
                self.isUserInteractionEnabled = false
            }
