...
//  Judou
//
//  Created by 4work on 2018/12/20.
...
#ifndef JS_SWAP // swap two value
#define JS_SWAP(_a_, _b_)  do { __typeof__(_a_) _tmp_ = (_a_); (_a_) = (_b_); (_b_) = _tmp_; } while (0)
#endif

static NSTimeInterval _js_CGImageSourceGetGIFFrameDelayAtIndex(CGImageSourceRef source, size_t index) {
...
static inline CGFloat JSDegreesToRadians(CGFloat degrees) {
    return degrees * M_PI / 180;
}

CGRect JSCGRectFitWithContentMode(CGRect rect, CGSize size, UIViewContentMode mode) {
...
+ (UIImage *)imageWithSmallGIFData:(NSData *)data scale:(CGFloat)scale {
    ...
    for (size_t i = 0; i < count; i++) {
        NSTimeInterval delay = _js_CGImageSourceGetGIFFrameDelayAtIndex(source, i);
        totalTime += delay;
...
+ (UIImage *)imageWithPDF:(id)dataOrPath {
    return [self _js_imageWithPDF:dataOrPath resize:NO size:CGSizeZero];
}

+ (UIImage *)imageWithPDF:(id)dataOrPath size:(CGSize)size {
    return [self _js_imageWithPDF:dataOrPath resize:YES size:size];
}
...
+ (UIImage *)_js_imageWithPDF:(id)dataOrPath resize:(BOOL)resize size:(CGSize)size {
...
- (void)drawInRect:(CGRect)rect withContentMode:(UIViewContentMode)contentMode clipsToBounds:(BOOL)clips{
    CGRect drawRect = JSCGRectFitWithContentMode(rect, self.size, contentMode);
...
- (UIImage *)_js_flipHorizontal:(BOOL)horizontal vertical:(BOOL)vertical {
...
- (UIImage *)imageByRotateLeft90 {
    return [self imageByRotate:JSDegreesToRadians(90) fitSize:YES];
}
...
- (UIImage *)imageByRotate180 {
    return [self _js_flipHorizontal:YES vertical:YES];
}

- (UIImage *)imageByFlipVertical {
    return [self _js_flipHorizontal:NO vertical:YES];
}

- (UIImage *)imageByFlipHorizontal {
    return [self _js_flipHorizontal:YES vertical:NO];
}
...
- (UIImage *)imageByBlurRadius:(CGFloat)blurRadius
    ...
    if (self.size.width < 1 || self.size.height < 1) {
        NSLog(@"UIImage+Unitl error: invalid size: (%.2f x %.2f). Both dimensions must be >= 1: %@", self.size.width, self.size.height, self);
        return nil;
    }
    if (!self.CGImage) {
        NSLog(@"UIImage+Unitl error: inputImage must be backed by a CGImage: %@", self);
        return nil;
    }
    if (maskImage && !maskImage.CGImage) {
        NSLog(@"UIImage+Unitl error: effectMaskImage must be backed by a CGImage: %@", maskImage);
        return nil;
    }
    ...
    if (!hasBlur && !hasSaturation) {
        return [self _js_mergeImageRef:imageRef tintColor:tintColor tintBlendMode:tintBlendMode maskImage:maskImage opaque:opaque];
    }
    ...
    if (hasNewFunc) {
        ...
        if (err != kvImageNoError) {
            NSLog(@"UIImage+Unitl error: vImageBuffer_InitWithCGImage returned error code %zi for inputImage: %@", err, self);
            return nil;
        }
        err = vImageBuffer_Init(&scratch, effect.height, effect.width, format.bitsPerPixel, kvImageNoFlags);
        if (err != kvImageNoError) {
            NSLog(@"UIImage+Unitl error: vImageBuffer_Init returned error code %zi for inputImage: %@", err, self);
            return nil;
        }
    } else {
    ...
    if (hasNewFunc) {
        ...
        effectCGImage = vImageCreateCGImageFromBuffer(input, &format, &_js_cleanupBuffer, NULL, kvImageNoAllocate, NULL);
        if (effectCGImage == NULL) {
            effectCGImage = vImageCreateCGImageFromBuffer(input, &format, NULL, NULL, kvImageNoFlags, NULL);
            free(input->data);
        }
        free(output->data);
        outputImage = [self _js_mergeImageRef:effectCGImage tintColor:tintColor tintBlendMode:tintBlendMode maskImage:maskImage opaque:opaque];
...
static void _js_cleanupBuffer(void *userData, void *buf_data) {
    free(buf_data);
}

// Helper function to add tint and mask.
- (UIImage *)_js_mergeImageRef:(CGImageRef)effectCGImage
                     tintColor:(UIColor *)tintColor
